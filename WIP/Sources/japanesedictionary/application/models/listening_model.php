...
class Listening_model extends CI_Model
{
	...
	function getListeningByLevel($level="",$off="",$limit=""){
		$table = '(SELECT l.lis_id,l.lis_title,l.lis_level,sc.sourcefile_id,sc.sourcefile_question,sc.sourcefile_script,sc.sourcefile_meaning,sc.sourcefile_answer 
            FROM traininglistening as l LEFT JOIN sourcefile sc ON l.lis_id=sc.lis_id WHERE lis_level=\''.mysql_real_escape_string(stripslashes($level)).'\' ORDER BY l.lis_id ASC) AS A';
		$this->db->select('*');
		$this->db->from($table);
		$this->db->limit($off,$limit);		
		$query = $this->db->get();
    ...
    function num_rowsBySearch($level=""){
	    $table = '(SELECT l.lis_id FROM traininglistening as l LEFT JOIN sourcefile sc ON l.lis_id=sc.lis_id WHERE lis_level=\''.mysql_real_escape_string(stripslashes($level)).'\' ORDER BY l.lis_id ASC) AS A';
        $this->db->from($table);
        return $this->db->count_all_results();
    }
    ...
    function getInfoTraininglistening($lis_id,$sourcefile_id){
    	...
    	$table = "";    	
    	if ($sourcefile_id != "") {
    		$table = '(SELECT l.lis_id,lis_title,lis_level,sc.sourcefile_id,sc.sourcefile_question,sc.sourcefile_script,sc.sourcefile_meaning,sc.sourcefile_answer FROM traininglistening as l LEFT JOIN sourcefile sc ON l.lis_id=sc.lis_id ORDER BY l.lis_id ASC) AS A';
