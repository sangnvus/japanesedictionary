...
class Traininglistening extends CI_Controller
{
	...
	function addTraininglistening(){
        ...
        if (isset($_POST['addnew'])) {
            ...
            $this->form_validation->set_rules("lis_title","Tiêu đề","required|max_length[500]|is_unique[traininglistening.lis_title]"); 
            $this->form_validation->set_rules("lis_level","Mức độ","required|max_length[10]");                    
    ...
    function editTraininglistening(){
        ...
        $data['error'] = "";
        if(!is_null($sourcefile_id) && $data['info']!=NULL)
        {            
            if(isset($_POST['ok']))
            {                       

                    $this->form_validation->set_rules("lis_id","Listening ID","required");
                    $this->form_validation->set_rules("lis_level","Mức độ","required|max_length[10]");
                    $this->form_validation->set_rules("lis_title","Tiêu đề","required|max_length[500]");
                    $this->form_validation->set_rules("sourcefile_id","Tên file","required|max_length[100]");                    
                    $this->form_validation->set_rules("sourcefile_question","Câu hỏi","required|max_length[500]");                
                    $this->form_validation->set_rules("sourcefile_script","Nội dung nghe","required|max_length[500]");                                          
                    $this->form_validation->set_rules("sourcefile_meaning","Dịch nghĩa","required|max_length[500]");
                    $this->form_validation->set_rules("sourcefile_answer","Đáp án","required|max_length[100]");
