...
class Conversation_model extends CI_Model
{
	...
	public function getDetailContentByCid($c_id){
		...
		// get from db
		//$c_id = intval($c_id);
		$txtQuery = '(SELECT c.c_id,c_level,con_id,con_title,con_hiragana,con_romaji,con_meaning FROM conversation as c LEFT JOIN conversationcontent cc ON c.c_id=cc.c_id WHERE c.c_id=\''.$c_id.'\')';
		$results = $this->db->query($txtQuery);
		if ($results->num_rows() > 0) {
			return $results->result();
		}
