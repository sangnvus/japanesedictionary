...
class Contact_model extends CI_Model
{
	...
	// Tong so record
    function num_rows(){
	    $table = '(SELECT contact_id FROM contact WHERE contact_status=1) AS A';        
        $this->db->from($table);
        return $this->db->count_all_results();
    }
    // get all data by Search
	function getContactByType($txtType,$off="",$limit=""){
		$table = '(SELECT * FROM contact WHERE contact_type = \''.$txtType.'\') AS A';		
		$this->db->from($table);
		$this->db->limit($off,$limit);		
		$query = $this->db->get();
        $data = $query->result_array();
    ...
    function num_rowsBySearch($txtType){
	    $table = '(SELECT contact_id FROM contact WHERE contact_type = \''.$txtType.'\') AS A';        
        $this->db->from($table);
        return $this->db->count_all_results();
    }
    //delete Contact
