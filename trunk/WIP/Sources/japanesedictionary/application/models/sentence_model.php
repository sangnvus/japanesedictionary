...
class Sentence_model extends CI_Model
{
	...
	function getByRomaji($txtRomaji,$off="",$limit=""){
		$table = '(SELECT * FROM sentence WHERE s_romaji LIKE \'%'.mysql_real_escape_string($txtRomaji).'%\') AS A';		
		$this->db->from($table);
		$this->db->limit($off,$limit);		
		$query = $this->db->get();
        $data = $query->result_array();
    ...
    function num_rowsBySearch($txtRomaji){
	    $table = '(SELECT s_id FROM sentence WHERE s_romaji LIKE \'%'.mysql_real_escape_string($txtRomaji).'%\') AS A';		       
        $this->db->from($table);
        return $this->db->count_all_results();
    }
    // Add New Sentence
