...
class Sentence_model extends CI_Model
{
	...
	function getAllSentence($off="",$limit=""){
		$table = '(SELECT * FROM sentence ORDER BY s_hiragana) AS A';		
		$this->db->from($table);
	...
	function getByRomaji($txtRomaji,$off="",$limit=""){
		$table = '(SELECT * FROM sentence WHERE s_romaji LIKE \'%'.mysql_real_escape_string($txtRomaji).'%\' ORDER BY s_hiragana) AS A';		
		$this->db->from($table);
		$this->db->limit($off,$limit);		
		$query = $this->db->get();
        $data = $query->result_array();
