...
class ReadingDocument_model extends CI_Model
{
    ...
    function getReadingDocument(){
        ...
        $table = '(SELECT * FROM readingdocument WHERE reading_id LIKE \'%'.'SC'.'%\') AS A';
        $this->db->select('*');
        $this->db->from($table);            
        $query = $this->db->get();
    ...
    function get50Vocabulary($off="",$limit=""){
        $table = '(SELECT readingvocabulary_hiragana,readingvocabulary_meaning,readingvocabulary_kanji 
            FROM readingvocabulary WHERE reading_id LIKE \'%'.'SC'.'%\'  ORDER BY reading_id ASC) AS A';
        $this->db->from($table);
        $this->db->limit($off,$limit);      
        $query = $this->db->get();
        $data = $query->result_array();
 ...
 // tong số 50 bài
    function num_rows_list50(){
        $table = '(SELECT readingvocabulary_id FROM readingvocabulary WHERE reading_id LIKE \'%'.'SC_'.'%\') AS A';        
        $this->db->from($table);
	...
	function getReadingByLevel($level="",$off="",$limit=""){
		$table = '(SELECT rd.reading_id,rd.reading_title,rd.reading_level,rv.readingvocabulary_id,rv.readingvocabulary_hiragana,rv.readingvocabulary_meaning,rv.readingvocabulary_kanji,rv.readingvocabulary_type 
			FROM readingdocument as rd LEFT JOIN readingvocabulary rv ON rd.reading_id=rv.reading_id WHERE reading_level= \''.mysql_real_escape_string(stripslashes($level)).'\' ORDER BY rd.reading_id ASC) AS A';
	...
	function getReadingArticleByLevel($level="",$off="",$limit=""){
		...
		$this->db->select('*');
		$this->db->where("reading_level",mysql_real_escape_string(stripslashes($level)));
		//$this->db->where("reading_type",'article');
		$this->db->from($table);
		$this->db->limit($off,$limit);		
		$query = $this->db->get();
    ...
    function num_rowsArticleBySearch($level=""){
	    $table = '(SELECT rd.reading_id
			FROM readingdocument as rd LEFT JOIN readingarticle ra ON rd.reading_id=ra.reading_id WHERE reading_level= \''.mysql_real_escape_string(stripslashes($level)).'\' ORDER BY rd.reading_id ASC) AS A';
		//$this->db->where("reading_type",'vocabulary');
        $this->db->from($table);
        return $this->db->count_all_results();
    }
