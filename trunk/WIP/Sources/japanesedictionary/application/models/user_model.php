...
class User_model extends CI_Model
{
    ...
    function getUserByUsername($username,$email){
        ...
        // get from db                
        if ($username !=="" && $email === "") {                
            $txtQuery = 'SELECT * FROM user WHERE u_username LIKE \'%' . $username.'%\' AND u_role=\'2\'';    
        } 
        if ($username ==="" && $email !== "") {            
            $txtQuery = 'SELECT * FROM user WHERE u_email LIKE \'%' . $email.'%\' AND u_role=\'2\'';
        }
        if ($username !=="" && $email !== "") {            
            $txtQuery = 'SELECT * FROM user WHERE u_username LIKE \'%'.$username.'%\' AND u_email LIKE \'%'.$email.'%\' AND u_role=\'2\'';
        }
        $results = $this->db->query($txtQuery);
        if ($results->num_rows() > 0) {
            return $results->result();
...
public function addUser($user)
    ...
    // add opinion
 public function add_contactopinion($data)
 {
