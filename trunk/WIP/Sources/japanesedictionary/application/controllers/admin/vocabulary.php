...
class Vocabulary extends CI_Controller
{
       ...
       $v_id = $this->uri->segment(4);
        ...
        {            
            ...
            {                         
                    ...
                    $this->form_validation->set_rules("m_specialized","m_specialized","max_length[200]");                                                                
    ...
    function addVocabulary(){
        ...
        if (isset($_POST['addnew'])) {
            ...
            $this->form_validation->set_rules("v_hiragana","V_hiragana","required|max_length[200]|callback_checkVocabulary");
            $this->form_validation->set_rules("v_romaji","V_romaji","required|max_length[200]");       
        if($this->form_validation->run()==FALSE){            
    ...
    function checkVocabulary($v_hiragana)
    {
        ...
        if($this->Vocabulary_model->getVocabulary($v_hiragana,$v_id)==TRUE){
            ...
            $this->form_validation->set_message("checkVocabulary","Từ đã tồn tại, vui lòng thử lại");
            return FALSE;
       }
    }
    function addMeaning(){
    ...
    function getReferByMid(){
        ...
        if (isset($_GET['txtMid'])) {
            $txtMid = intval(trim($_GET['txtMid']));            
        }
          
        if(is_numeric($txtMid)){
    ...
    function addRefer(){
        ...
        {                                
                ...
                if ($this->Vocabulary_model->checkMid_Sid($this->input->post("m_id"),$this->input->post("s_id")) == TRUE) {
                    $this->load->view("admin/vocabulary/addReferSentence_view",array("error"=>"Existed!"));
                } else {
                    try {
