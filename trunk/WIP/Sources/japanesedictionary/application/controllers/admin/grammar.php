...
class Grammar extends CI_Controller
{
    ...
    function approvedGrammar(){
        ...
        {            
            if(isset($_POST['ok']))
            {                                               
                    $this->form_validation->set_rules("g_hiragana","g_hiragana","required|is_unique[grammar.g_hiragana]|max_length[200]");
                    $this->form_validation->set_rules("g_romaji","g_romaji","required|max_length[200]");
                    $this->form_validation->set_rules("g_level","g_level","required|max_length[10]");
                    $this->form_validation->set_rules("g_meaning","g_meaning","required|max_length[200]");
                    $this->form_validation->set_rules("g_use","g_use","required|max_length[1000]");
                    $this->form_validation->set_rules("reading_id", "reading_id", 'trim|max_length[20]');                                                                               
	...
	function addGrammar(){
        ...
        if (isset($_POST['addnew'])) {
            ...
            $this->form_validation->set_rules("g_hiragana","Ngữ pháp","required|is_unique[grammar.g_hiragana]|max_length[200]");
            $this->form_validation->set_rules("g_romaji","g_romaji","required|max_length[200]");
            $this->form_validation->set_rules("g_level","g_level","required|max_length[10]");
            $this->form_validation->set_rules("g_meaning","g_meaning","required|max_length[200]");
            $this->form_validation->set_rules("g_use","g_use","required|max_length[1000]");
            $this->form_validation->set_rules("reading_id", "reading_id", 'trim|max_length[20]|alpha_numeric');                          
        if($this->form_validation->run()==FALSE){            
	...
	function editGrammar(){
        ...
        {            
            if(isset($_POST['ok']))
            {           
                $this->form_validation->set_rules("g_hiragana","Ngữ pháp","required|max_length[200]");
                $this->form_validation->set_rules("g_romaji","g_romaji","required|max_length[200]");
                $this->form_validation->set_rules("g_level","g_level","required|max_length[10]");
                $this->form_validation->set_rules("g_meaning","g_meaning","required|max_length[200]");
                $this->form_validation->set_rules("g_use","g_use","required|max_length[1000]");
    ...
    function getReferByGid(){
        ...
        if (isset($_GET['txtGid'])) {
            $txtGid = stripslashes(trim($_GET['txtGid']));            
        }
        if(is_numeric($txtGid)){
    ...
    function addRefer(){
        ...
        if (isset($_POST['addnew'])) {
            ...
            $this->form_validation->set_rules("g_id","g_id","required|callback_checkExistGid");
        ...
        {                
                ...
                if ($this->Grammar_model->checkGid_Sid($this->input->post("g_id"),$this->input->post("s_id")) == TRUE) {
                    $this->load->view("admin/grammar/addReferSentence_view",array("error"=>"Existed!"));                    
                } else {
                    try {
    ...
    function checkExistGid($g_id){        
        if($this->Grammar_model->checkGid($g_id)==TRUE){
            $this->form_validation->set_message("checkExistGid","G_id not exist!");
            return FALSE;
        }
        else{
            return TRUE;
    ...
    function checkExistSid($s_id){        
        if($this->Grammar_model->checkSid($s_id)==TRUE){
        ...
        {            
            ...
            {                            
                ...
                if($this->form_validation->run()==FALSE){   
                    ...
                    if ($g_id == $this->input->post("g_id") && $s_id == $this->input->post("s_id")) {
                                redirect(base_url()."index.php/admin/grammar/referenceSentence");                 
                            } else {
                                if ($this->Grammar_model->checkGid_Sid($this->input->post("g_id"),$this->input->post("s_id")) == TRUE) {
                                    $this->load->view("admin/grammar/editRefer_view",array("error"=>"(G_id,S_id) existed!","g_id" => $g_id,"s_id"=>$s_id));
                                } else {
                                    try {
