...
class Grammar extends CI_Controller
{
	...
	function getByRomaji(){
        ...
        if($max!=0){    
            ...
            $this->load->view("admin/grammar/listSearchGrammar_view",$data);
        }else{
        	$data['grammar'] = null;
            $this->load->view("admin/grammar/listSearchGrammar_view",$data);
        }
	...
	function addGrammar(){
        ...
        if (isset($_POST['addnew'])) {
            ...
            $this->form_validation->set_rules("g_status","g_status","required");
            //$this->form_validation->set_rules("reading_id","reading_id","required");                            
        ...
        {                
                $grammar = array(                        
                        ...
                        "g_status"  => $this->input->post("g_status"),
    ...
    function getReferByGid(){
        ...
        if($max!=0){    
            ...
            $data['links'] = $this->pagination->create_links();
            //$data['referSentence'] = $this->Grammar_model->getReferByGid($txtGid,$min,$this->uri->segment($config['uri_segment']));
            $referSentence = $this->Grammar_model->getReferByGid($txtGid,$min,$this->uri->segment($config['uri_segment']));
           /* echo "<pre>";
            print_r($referSentence);
            echo "</pre>";
            die;*/
