...
class Grammar_model extends CI_Model
{
    ...
    function num_rowsBySearch($txtRomaji){
        $table = '(SELECT g_id FROM grammar WHERE g_status = 1 AND g_romaji LIKE \'%'.$txtRomaji.'%\') AS A';             
        $this->db->from($table);
        return $this->db->count_all_results();
    }
    ...
    function getGrammarByReadingId($reading_id){
        ...
        $table ='SELECT * FROM grammar WHERE reading_id = \''.$reading_id.'\'';
        /*$this->db->from($table);        
        $query = $this->db->get();
        $data = $query->result_array();
        return $data;*/
    ...
    function getSentenByGrammarId($g_id){
        $table = 'SELECT * FROM grammarsentence as gs JOIN sentence as s ON gs.s_id=s.s_id WHERE gs.g_id=\''.$g_id.'\'';
        /*$this->db->from($table);        
        $query = $this->db->get();
        $data = $query->result_array();
        return $data;*/
